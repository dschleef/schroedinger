...
static GstStateChangeReturn gst_base_video_codec_change_state (GstElement *element,
...
{
  GST_DEBUG_CATEGORY_INIT (basevideo_debug, "basevideo", 0, "Base Video Classes");
...
static void
gst_base_video_codec_init (GstBaseVideoCodec *base_video_codec, GstBaseVideoCodecClass *klass)
{
  ...
  gst_element_add_pad (GST_ELEMENT(base_video_codec), base_video_codec->sinkpad);
  ...
  gst_element_add_pad (GST_ELEMENT(base_video_codec), base_video_codec->srcpad);
...
gst_base_video_codec_src_event (GstPad *pad, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_QOS:
    {
      ...
      GST_DEBUG_OBJECT (base_video_codec, "got QoS %" GST_TIME_FORMAT ", %" G_GINT64_FORMAT,
...
gst_base_video_codec_sink_event (GstPad *pad, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_FLUSH_STOP:
      ...
      break;
    case GST_EVENT_EOS:
      if (gst_base_video_codec_push_all (base_video_codec, FALSE) == GST_FLOW_ERROR) {
        gst_event_unref (event);
        return FALSE;
      }
...
gst_base_video_codec_change_state (GstElement *element, GstStateChange transition)
{
