...
gst_base_video_parse_src_query (GstPad *pad, GstQuery *query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:
      res = gst_pad_query (GST_PAD_PEER (
            GST_BASE_VIDEO_CODEC_SINK_PAD(base_parse)), query);
      if (!res) goto error;
      break;
...
gst_base_video_parse_src_event (GstPad *pad, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEEK:
    {
      ...
      res = gst_pad_push_event (GST_BASE_VIDEO_CODEC_SINK_PAD(base_video_parse), real_seek);
...
#endif
    default:
      res = gst_pad_push_event (GST_BASE_VIDEO_CODEC_SINK_PAD(base_video_parse), event);
      break;
  }
done:
  gst_object_unref (base_video_parse);
...
gst_base_video_parse_sink_event (GstPad *pad, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_FLUSH_START:
      ret = gst_pad_push_event (GST_BASE_VIDEO_CODEC_SRC_PAD(base_video_parse), event);
      break;
    case GST_EVENT_FLUSH_STOP:
      ...
      break;
    case GST_EVENT_EOS:
      if (gst_base_video_parse_push_all (base_video_parse, FALSE) == GST_FLOW_ERROR) {
        gst_event_unref (event);
        return FALSE;
      }
    ...
    GstCaps *caps)
{
  ...
  return gst_pad_set_caps (GST_BASE_VIDEO_CODEC_SRC_PAD(base_video_parse), caps);
}
