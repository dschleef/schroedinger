...
static gboolean gst_base_video_parse_sink_query (GstPad *pad, GstQuery *query);
static gboolean gst_base_video_parse_src_event (GstPad *pad, GstEvent *event);
static gboolean gst_base_video_parse_sink_event (GstPad *pad, GstEvent *event);
static GstStateChangeReturn gst_base_video_parse_change_state (GstElement *element,
    GstStateChange transition);
static GstFlowReturn gst_base_video_parse_push_all (GstBaseVideoParse *base_video_parse, 
    gboolean at_eos);
...
GST_BOILERPLATE (GstBaseVideoParse, gst_base_video_parse, GstElement,
    GST_TYPE_ELEMENT);
...
static void
gst_base_video_parse_init (GstBaseVideoParse *base_video_parse, GstBaseVideoParseClass *klass)
{
  ...
  gst_pad_set_chain_function (base_video_parse->sinkpad, gst_base_video_parse_chain);
  gst_pad_set_query_function (base_video_parse->sinkpad, gst_base_video_parse_sink_query);
  gst_pad_set_event_function (base_video_parse->sinkpad, gst_base_video_parse_sink_event);
  gst_element_add_pad (GST_ELEMENT(base_video_parse), base_video_parse->sinkpad);
  ...
  gst_pad_set_query_type_function (base_video_parse->srcpad, gst_base_video_parse_get_query_types);
  gst_pad_set_query_function (base_video_parse->srcpad, gst_base_video_parse_src_query);
  gst_pad_set_event_function (base_video_parse->srcpad, gst_base_video_parse_src_event);
  gst_element_add_pad (GST_ELEMENT(base_video_parse), base_video_parse->srcpad);
...
gst_base_video_parse_src_query (GstPad *pad, GstQuery *query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:
      res = gst_pad_query (GST_PAD_PEER (base_parse->sinkpad), query);
      if (!res) goto error;
      break;
...
gst_base_video_parse_src_event (GstPad *pad, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEEK:
    {
      ...
      res = gst_pad_push_event (base_video_parse->sinkpad, real_seek);
    ...
    case GST_EVENT_QOS:
    {
      ...
      GST_DEBUG_OBJECT (base_video_parse, "got QoS %" GST_TIME_FORMAT ", %" G_GINT64_FORMAT,
...
gst_base_video_parse_sink_event (GstPad *pad, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_FLUSH_START:
      ret = gst_pad_push_event (base_parse->srcpad, event);
      break;
    case GST_EVENT_FLUSH_STOP:
      ...
      break;
    case GST_EVENT_EOS:
      if (gst_base_video_parse_push_all (base_parse, FALSE) == GST_FLOW_ERROR) {
        gst_event_unref (event);
        return FALSE;
      }
    ...
    case GST_EVENT_NEWSEGMENT:
    {
      ...
      gst_segment_set_newsegment (&base_parse->state.segment, update,
...
newseg_wrong_format:
  GST_DEBUG_OBJECT (base_parse, "received non TIME newsegment");
  gst_event_unref (event);
  goto done;

newseg_wrong_rate:
  GST_DEBUG_OBJECT (base_parse, "negative rates not supported");
  gst_event_unref (event);
  goto done;
}
...
gst_base_video_parse_change_state (GstElement *element, GstStateChange transition)
{
  ...
  if (picture_number < 0) {
    return base_video_parse->timestamp_offset -
      (gint64)gst_util_uint64_scale (-picture_number,
          base_video_parse->state.fps_d * GST_SECOND, base_video_parse->state.fps_n);
  } else {
    return base_video_parse->timestamp_offset +
...
gst_base_video_parse_push_all (GstBaseVideoParse *base_video_parse, gboolean at_eos)
{
    ...
    GstCaps *caps)
{
  ...
  return gst_pad_set_caps (base_video_parse->srcpad, caps);
}
    ...
    GstBuffer *buffer)
{
  ...
  if (base_video_parse->caps == NULL) {
    ...
    base_video_parse->caps = base_video_parse_class->get_caps (base_video_parse);

    ret = gst_pad_set_caps (base_video_parse->srcpad, base_video_parse->caps);

    if (!ret) {
      GST_ERROR("pad didn't accept caps");
  ...
  return gst_pad_push (base_video_parse->srcpad, buffer);
}
