...
static void gst_logoinsert_class_init (gpointer g_class,
    gpointer class_data);
...
static void gst_logoinsert_set_location (GstLogoinsert *li, const gchar *location);

static gboolean gst_logoinsert_set_caps (GstBaseTransform *base_transform,
    GstCaps *incaps, GstCaps *outcaps);
static GstFlowReturn gst_logoinsert_transform_ip (GstBaseTransform * base_transform,
    GstBuffer *buf);
static SchroFrame * schro_frame_new_from_png (void *data, int size);
static SchroFrame * schro_virt_frame_extract_alpha_take (SchroFrame *frame);
static SchroFrame * schro_frame_realize (SchroFrame *frame);
    ...
    GstCaps *incaps, GstCaps *outcaps)
{
  ...
  gst_video_format_parse_caps (incaps, &li->format, &li->width,
      &li->height);
...
gst_logoinsert_transform_ip (GstBaseTransform * base_transform,
    GstBuffer *buf)
{
  GstLogoinsert *li;
  SchroFrame *frame;
  ...
  frame = gst_schro_buffer_wrap (buf, li->format, li->width, li->height);

  if (li->ayuv_frame == NULL) return GST_FLOW_OK;

  if (li->overlay_frame == NULL) {
    SchroFrame *f;

    f = schro_virt_frame_extract_alpha_take (
        schro_frame_ref (li->ayuv_frame));
    f = schro_virt_frame_new_subsample_take (f, frame->format);
    li->alpha_frame = schro_frame_realize (f);

    f = schro_virt_frame_new_unpack_take (schro_frame_ref (li->ayuv_frame));
    f = schro_virt_frame_new_color_matrix_take (f);
    f = schro_virt_frame_new_subsample_take (f, frame->format);
    li->overlay_frame = schro_frame_realize (f);
  }
  ...
  if (1) {
    ...
    for(k=0;k<3;k++){
      ...
      for(j=0;j<li->overlay_frame->components[k].height;j++){
        dest = SCHRO_FRAME_DATA_GET_LINE (frame->components + k, j + offset_y);
        src = SCHRO_FRAME_DATA_GET_LINE (li->overlay_frame->components + k, j);
        alpha = SCHRO_FRAME_DATA_GET_LINE (li->alpha_frame->components + k, j);

#define oil_divide_255(x) ((((x)+128) + (((x)+128)>>8))>>8)

        for(i=0;i<li->overlay_frame->components[k].width;i++){
          dest[i+offset_x] = oil_divide_255 (src[i]*alpha[i] + dest[i+offset_x]*(255-alpha[i]));
        }
      }
    }
  }
...
gst_logoinsert_set_location (GstLogoinsert *li, const gchar *location)
{
  ...
  g_free (li->data);
  if (li->overlay_frame) {
    schro_frame_unref (li->overlay_frame);
  ...
  li->ayuv_frame = schro_frame_new_from_png (li->data, li->size);

  if (li->alpha_frame) {
    schro_frame_unref (li->alpha_frame);
    li->alpha_frame = NULL;
  }
  if (li->overlay_frame) {
    schro_frame_unref (li->overlay_frame);
...
static SchroFrame *
schro_frame_new_from_png (void *data, int size)
{
  ...
  frame = schro_frame_new_from_data_AYUV (frame_data, width, height);

  rowbytes = png_get_rowbytes (png_ptr, info_ptr);
  rows = (png_bytep *) g_malloc (sizeof (png_bytep) * height);

  for (j = 0; j < height; j++) {
    rows[j] = SCHRO_FRAME_DATA_GET_LINE (frame->components+0, j);
  }
...
static void
extract_alpha (SchroFrame *frame, void *_dest, int component, int j)
{
  uint8_t *dest = _dest;
  uint8_t *src;
  int i;

  src = SCHRO_FRAME_DATA_GET_LINE (frame->virt_frame1->components + 0, j);
...
static SchroFrame *
schro_frame_realize (SchroFrame *frame)
{
  SchroFrame *dest;

  dest = schro_frame_clone (NULL, frame);
