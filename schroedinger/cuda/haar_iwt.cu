...
static __global__ void a_transform_h( DATATYPE* data, int width, int stride, int shift )
{
    ...
    {
        uint32_t val = row[ofs];
        shared[ofs]        = (val&0xFFFF) << shift; // even
        shared[ofs + half] = (val>>16) << shift; // odd
    }
    ...
    for(ofs = tid; ofs < (width>>1); ofs += BSH)
        row[ofs] = *((uint32_t*)&shared[ofs<<1]);
}

// Process BROWS lines
__device__ void doTransform()
{
...
void cuda_iwt_haar(int shift, int16_t *d_data, int lwidth, int lheight, int stride)
{
    /** Invoke kernel */
    dim3 block_size;
    dim3 grid_size;
    ...
    a_transform_h<<<grid_size, block_size, shared_size>>>(d_data, lwidth, stride, shift);
#endif
#ifdef VERTICAL
    block_size.x = BSVX;
    block_size.y = BSVY;
