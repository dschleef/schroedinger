...
__device__ worktype tex_u8(RefTex tex, float x, float y)
{
    return (worktype)(tex2D(tex, x, y)*256.0f);
}
...
__device__ worktype2 do_motion_block(short bidx, short bidy, float xofs, float yofs, short comp_shift, float sxscale, float syscale,
  short weight1, short weight2, short weight_shift)
{
    ...
    //short4 mv = tex2D(bt1, (float)1, (float)1);
    //short4 mv = make_short4(0,MOTION_NONE,0,0);
    //int4 mv = __itexfetchi(bt1, make_int4(bidx, bidy, 0, 0));
    ...
    if(mv.x != MOTION_NONE)
    {
        if(mv.y != MOTION_NONE) // Use both references
        {
            ...
            ref1_v = tex_u8(ref1, xofs + mv.x * sxscale, yofs + mv.z * syscale);
            ref2_v = tex_u8(ref2, xofs + mv.y * sxscale, yofs + mv.w * syscale);
            val.x = (__mul24(ref1_v, weight1) +
                     __mul24(ref2_v, weight2)) >> weight_shift;
            
            ref1_v = tex_u8(ref1, xofs + mv.x * sxscale + PDIFF, yofs + mv.z * syscale);
            ref2_v = tex_u8(ref2, xofs + mv.y * sxscale + PDIFF, yofs + mv.w * syscale);
            val.y = (__mul24(ref1_v, weight1) +
                     __mul24(ref2_v, weight2)) >> weight_shift;
        }
        else // Use only reference 1
        {
            val.x = tex_u8(ref1, xofs + mv.x * sxscale, yofs + mv.z * syscale);
            val.y = tex_u8(ref1, xofs + mv.x * sxscale + PDIFF, yofs + mv.z * syscale);
        }
    }
    else
    {
        ...
        {
            val.x = tex_u8(ref2, xofs + mv.y * sxscale, yofs + mv.w * syscale);
            val.y = tex_u8(ref2, xofs + mv.y * sxscale + PDIFF, yofs + mv.w * syscale);
        }
        else // Only DC
        {
...
__global__ void motion_copy_2ref(
    uint16_t* dest, int dstride, int dwidth, int dheight, int xB, int yB,
    // Component parameters
    short comp /* times 8 */, float sxscale /* exp2f(-x_shift) * 0.25f */ , float syscale /* exp2f(-y_shift) * 0.25f */,
    ...
    if(blockIdx.x < xB) 
    {
        if(blockIdx.y < yB) // 1: Inner (no blend)
        {
            ...
            if(xofs >= 0 && yofs >= 0 && xofs < dwidth && yofs < dheight) /* Clip to image */
            {
                ...
                val = do_motion_block(obx, oby, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
        ...
        else  // 3: Vertical (blend two blocks)
        {
            ...
            if(xofs >= 0 && yofs >= 0 && xofs < dwidth && yofs < dheight) /* Clip to image */
            {
                ...
                worktype2 valy1 = do_motion_block(obx, oby, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
                worktype2 valy2 = do_motion_block(obx, oby+1, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
    ...
    else
    {
        if(blockIdx.y < yB) // 2: Horizontal (blend 2 blocks)
        {
            ...
            if(xofs >= 0 && yofs >= 0 && xofs < dwidth && yofs < dheight) /* Clip to image */
            {
                ...
                worktype2 valx1 = do_motion_block(obx, oby, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
                worktype2 valx2 = do_motion_block(obx+1, oby, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
        ...
        else // Diagonal (blend 4 blocks)
        {
            ...
            if(xofs >= 0 && yofs >= 0 && xofs < dwidth && yofs < dheight) /* Clip to image */
            {
                ...
                {
                    worktype2 valx1y1 = do_motion_block(obx, oby, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
                    worktype2 valx2y1 = do_motion_block(obx+1, oby, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
                    valy1.x = __mul24(xw1, valx1y1.x) + __mul24(xw2, valx2y1.x);
                    valy1.y = __mul24(xw1, valx1y1.y) + __mul24(xw2, valx2y1.y);
                }
                {
                    worktype2 valx1y2 = do_motion_block(obx, oby+1, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
                    worktype2 valx2y2 = do_motion_block(obx+1, oby+1, xofs*PDIFF + POFS, yofs*PDIFF + POFS, comp, sxscale, syscale, weight1, weight2, weight_shift);
