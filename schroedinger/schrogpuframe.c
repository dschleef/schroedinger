...
void schro_gpuframe_convert (SchroFrame *dest, SchroFrame *src)
{
     ...
     (src->format==SCHRO_FRAME_FORMAT_S16_420 && dest->format==SCHRO_FRAME_FORMAT_U8_420))
  {
      // S16 to U8
      for(i=0; i<3; ++i)
          cuda_convert_u8_s16(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                              src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
          ...
          (src->format==SCHRO_FRAME_FORMAT_U8_420 && dest->format==SCHRO_FRAME_FORMAT_S16_420))
  {
      // U8 to S16
      for(i=0; i<3; ++i)
          cuda_convert_s16_u8(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                              src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
          ...
          (src->format==SCHRO_FRAME_FORMAT_U8_420 && dest->format==SCHRO_FRAME_FORMAT_U8_420))
  {
      // U8 to U8
      for(i=0; i<3; ++i)
          cuda_convert_u8_u8(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                             src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
          ...
          (src->format==SCHRO_FRAME_FORMAT_S16_420 && dest->format==SCHRO_FRAME_FORMAT_S16_420))
  {
      // S16 to S16
      for(i=0; i<3; ++i)
          cuda_convert_s16_s16(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                               src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
  else if(src->format==SCHRO_FRAME_FORMAT_YUYV && dest->format==SCHRO_FRAME_FORMAT_U8_422)
  {
      // deinterleave YUYV
      cuda_convert_u8_422_yuyv(dest->components[0].data, dest->components[0].stride,
                               ...
                               src->width, src->height, CUDA_STREAM);
  }
  else if(src->format==SCHRO_FRAME_FORMAT_UYVY && dest->format==SCHRO_FRAME_FORMAT_U8_422)
  {
      // deinterleave UYVY
      cuda_convert_u8_422_uyvy(dest->components[0].data, dest->components[0].stride,
                               dest->components[1].data, dest->components[1].stride,
                               dest->components[2].data, dest->components[2].stride,
                               dest->width, dest->height,
                               src->components[0].data, src->components[0].stride,
  ...
  {
      // deinterleave AYUV
      cuda_convert_u8_444_ayuv(dest->components[0].data, dest->components[0].stride,
                               dest->components[1].data, dest->components[1].stride,
                               dest->components[2].data, dest->components[2].stride,
                               dest->width, dest->height,
                               src->components[0].data, src->components[0].stride,
  ...
  {
      // interleave YUYV
      cuda_convert_yuyv_u8_422(dest->components[0].data, dest->components[0].stride,
                               dest->width, dest->height,
                               src->components[0].data, src->components[0].stride,
                               src->components[1].data, src->components[1].stride,
                               src->components[2].data, src->components[2].stride,
                               src->width, src->height, CUDA_STREAM);
  }
  else if(src->format==SCHRO_FRAME_FORMAT_U8_422 && dest->format==SCHRO_FRAME_FORMAT_UYVY)
  {
      // interleave UYVY
      cuda_convert_uyvy_u8_422(dest->components[0].data, dest->components[0].stride,
                               dest->width, dest->height,
                               src->components[0].data, src->components[0].stride,
                               src->components[1].data, src->components[1].stride,
                               src->components[2].data, src->components[2].stride,
  ...
  {
      // interleave AYUV
      cuda_convert_ayuv_u8_444(dest->components[0].data, dest->components[0].stride,
                               dest->width, dest->height,
                               src->components[0].data, src->components[0].stride,
                               src->components[1].data, src->components[1].stride,
                               src->components[2].data, src->components[2].stride,
...
void schro_gpuframe_add (SchroFrame *dest, SchroFrame *src)
{
     ...
     (src->format==SCHRO_FRAME_FORMAT_U8_420 && dest->format==SCHRO_FRAME_FORMAT_S16_420))
  {
      // U8 to S16
      for(i=0; i<3; ++i)
          cuda_add_s16_u8(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                          src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
  else if((src->format==SCHRO_FRAME_FORMAT_S16_444 && dest->format==SCHRO_FRAME_FORMAT_S16_444) ||
          (src->format==SCHRO_FRAME_FORMAT_S16_422 && dest->format==SCHRO_FRAME_FORMAT_S16_422) ||
          (src->format==SCHRO_FRAME_FORMAT_S16_420 && dest->format==SCHRO_FRAME_FORMAT_S16_420))
  {
      // S16 to S16
      for(i=0; i<3; ++i)
          cuda_add_s16_s16(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                           src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
  else 
  {
...
void schro_gpuframe_subtract (SchroFrame *dest, SchroFrame *src)
{
     ...
     (src->format==SCHRO_FRAME_FORMAT_U8_420 && dest->format==SCHRO_FRAME_FORMAT_S16_420))
  {
      // U8 to S16
      for(i=0; i<3; ++i)
          cuda_subtract_s16_u8(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                          src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
  else if((src->format==SCHRO_FRAME_FORMAT_S16_444 && dest->format==SCHRO_FRAME_FORMAT_S16_444) ||
          (src->format==SCHRO_FRAME_FORMAT_S16_422 && dest->format==SCHRO_FRAME_FORMAT_S16_422) ||
          (src->format==SCHRO_FRAME_FORMAT_S16_420 && dest->format==SCHRO_FRAME_FORMAT_S16_420))
  {
      // S16 to S16
      for(i=0; i<3; ++i)
          cuda_subtract_s16_s16(dest->components[i].data, dest->components[i].stride, dest->components[i].width, dest->components[i].height,
                           src->components[i].data, src->components[i].stride, src->components[i].width, src->components[i].height, CUDA_STREAM);
  }
  else 
  {
...
void schro_gpuframe_iwt_transform (SchroFrame *frame, SchroParams *params)
{
  ...
  for(component=0;component<3;component++){
    ...
    for(level=0;level<params->transform_depth;level++) {
      ...
      cuda_wavelet_transform_2d (params->wavelet_filter_index, frame_data, stride, w, h, CUDA_STREAM);
    }
  }
}
...
void schro_gpuframe_inverse_iwt_transform (SchroFrame *frame, SchroParams *params)
{
  ...
  for(component=0;component<3;component++){
    ...
    for(level=params->transform_depth-1; level >=0;level--) {
      ...
      cuda_wavelet_inverse_transform_2d (params->wavelet_filter_index, frame_data, stride, w, h, CUDA_STREAM);
    }
#ifdef TEST
    /// Copy frame from GPU
    cudaMemcpy(c_data, frame_data, height * comp->stride, cudaMemcpyDeviceToHost);
...
void schro_gpuframe_to_cpu (SchroFrame *dest, SchroFrame *src)
{
  ...
  SCHRO_ASSERT(!SCHRO_FRAME_IS_CUDA(dest));
       ...
       src->components[2].length == dest->components[2].length)
    {
        ...
        cudaMemcpy(dest->components[0].data, src->components[0].data, src->components[0].length+src->components[1].length+src->components[2].length, cudaMemcpyDeviceToHost);
    }
    else
    {
        for(i=0; i<3; ++i)
...
void schro_frame_to_gpu (SchroFrame *dest, SchroFrame *src)
{
  ...
  SCHRO_ASSERT(SCHRO_FRAME_IS_CUDA(dest));
    ...
    {
        SCHRO_DEBUG("consecutive %i+%i+%i", src->components[0].length, src->components[1].length, src->components[2].length);
        cudaMemcpy(dest->components[0].data, src->components[0].data, src->components[0].length+src->components[1].length+src->components[2].length, cudaMemcpyHostToDevice);
    }
    else
    {
        for(i=0; i<3; ++i)
...
void schro_gpuframe_compare (SchroFrame *a, SchroFrame *b)
{
  ...
  SCHRO_ASSERT(!SCHRO_FRAME_IS_CUDA(b));
    ...
    for(i=0; i<3; ++i)
    {
        ...
        cudaMemcpy(temp, a->components[i].data, a->components[i].length, cudaMemcpyDeviceToHost);
...
void schro_gpuframe_zero (SchroFrame *dest)
{
  ...
  SCHRO_ASSERT(SCHRO_FRAME_IS_CUDA(dest));
    ...
    if(dest->components[1].data == (dest->components[0].data + dest->components[0].length) &&
       dest->components[2].data == (dest->components[1].data + dest->components[1].length))
    {
        cudaMemset(dest->components[0].data, 0, dest->components[0].length+dest->components[1].length+dest->components[2].length);
    }
    else
    {
...
void schro_gpuframe_upsample(SchroFrame *dst, SchroFrame *src)
{
  ...
  SCHRO_ASSERT(SCHRO_FRAME_IS_CUDA(src));
    ...
    for(i=0; i<3; ++i)
    {
        ...
        cuda_upsample_horizontal(dst_data, dst_stride*2, src_data, src_stride, width, height, CUDA_STREAM);
        cuda_upsample_vertical(dst_data+dst_stride, dst_stride*2, dst_data, dst_stride*2, width*2, height, CUDA_STREAM);
    }
}
...
void schro_upsampled_gpuframe_upsample(SchroUpsampledFrame *rv, SchroFrame *temp_f, SchroFrame *src, SchroVideoFormat *fmt)
{
  ...
  SCHRO_ASSERT(SCHRO_FRAME_IS_CUDA(src));
    ...
    cudaMemcpy2DToArray(rv->components[0], 0, 0, temp_f->components[0].data, temp_f->components[0].stride, fmt->width*2, fmt->height*2, cudaMemcpyDeviceToDevice);
    cudaMemcpy2DToArray(rv->components[1], 0, 0, temp_f->components[1].data, temp_f->components[1].stride, fmt->chroma_width*2, fmt->chroma_height*2, cudaMemcpyDeviceToDevice);    
    cudaMemcpy2DToArray(rv->components[2], 0, 0, temp_f->components[2].data, temp_f->components[2].stride, fmt->chroma_width*2, fmt->chroma_height*2, cudaMemcpyDeviceToDevice);
